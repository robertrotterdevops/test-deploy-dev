...
```bash
kubectl get secret AKS-01-es-elastic-user -n AKS-01 -o jsonpath='{{.data.elastic}}' | base64 -d
```

### Port-forward Elasticsearch
```bash
kubectl port-forward svc/AKS-01-es-http -n AKS-01 9200:9200
```

### Port-forward Kibana
```bash
kubectl port-forward svc/AKS-01-kb-http -n AKS-01 5601:5601
```
